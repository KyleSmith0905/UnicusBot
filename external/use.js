module.exports = {
    ...
    execute (message, args) {
        try {
            ...
            const abilityFiles = fs.readdirSync ('./external/use').filter (file => file.endsWith('.js')); // Gets all Javascript files under /external/
            for (const file of abilityFiles) {
                const ability = require (`./use/${file}`); // Gets the file itself
